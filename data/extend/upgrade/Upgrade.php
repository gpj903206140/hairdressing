...
class Upgrade111
{
    ...
    */
     ...
     * 升级版本
     */
    public function shopping_patch_upgrade111()
    {
        /**
         * 下载补丁包
         */
        ...
        $this->sql_file_path=$this->download_update_file."/newshop_".$this->version_no."_patch.sql";
        /**
         * 导入升级数据库数据
         */
    ...
    public function create_upgrade_file111($patch_release){
        $upgrade_code=0;
        $upgrade_message=0;
        $download_file_path_dow=ROOT_PATH.'download/';
        $download_file_path_upgrade=ROOT_PATH.'download/upgrade/';
        $download_file_path=ROOT_PATH.'download/upgrade/'.'newshop_patch_'.$patch_release.'/';
     ...
     * 下载升级包
     */
    public function update_file_download111($download_url, $patch_release)
    {
        $upgrade_code=0;
     ...
     * 解压版本
     */
    public function update_file_unzip111($download_zip_path, $download_file_path, $download_update_file)
    {
        $upgrade_code=0;
        ...
        try {
             ...
             * 检索文件开始路径
             */
            $file_start_path=$download_update_file."/newshop_b2c/";
        } catch (\Exception $e) {
            $upgrade_code=-1;
     ...
     * 检索需要更新的文件
     */
    public function update_file_deal111($file_start_path)
    {
        $upgrade_code=0;
     ...
     * 得到需要更新的文件
     */
    private function get_update_file111($file_start_path){
     ...
     * 检测升级文件路径的权限
     */
    public function detect_file_permission111($deal_file_array){
        $upgrade_code=0;
     ...
     * 文件备份
     */
    public function update_file_backup1111($deal_file_array, $download_back_file){
        $upgrade_code=0;
     ...
     * 创建文件备份的文件夹
     */
    public function create_backup_file1111($file_path, $back_path){
        try {
            $file_path=str_replace("\\", "/", $file_path);
            $file_str=explode("/", $file_path);
     ...
     * 文件覆盖
     */
    public function update_file_cover111($deal_file_array, $file_start_path){
        $upgrade_code=0;
     ...
     * 导入sql文件
     */
    public function update_sql_execute111($download_update_file, $version_no)
    {
        $upgrade_code=0;
        $upgrade_message="数据库导入成功!";
        Db::startTrans();
        try {
            $sqlfile = $download_update_file."/newshop_".$version_no."_patch.sql";
     ...
     */
    public function execute_upgrade_sql111($download_update_file, $version_no){
        $is_lock=$download_update_file."/newshop_".$version_no."_patch.lock";
     ...
     */
    private function sql_execute111($sqlfile, $is_debug){
        $upgrade_code=0;
        $upgrade_message="数据库导入成功!";
        if(!$is_debug){
            Db::startTrans();
     ...
     * 更新失败 还原文件
     */
    public function update_file_regain111($download_back_file){
        $upgrade_code=0;
     ...
     * @return number
     */
    private function dirWriteable111($dir)
    {
        $writeable = 0;
        if(!is_dir($dir)) {
            @mkdir($dir, 0777, true);
