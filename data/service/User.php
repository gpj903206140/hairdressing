...
/**
    ...
    private function initLoginInfo($user_info)
    {
        ...
        Session::set($model.'is_promoters', $user_info['is_promoters']);
        //单店版本
    ...
    public function login($user_name, $password = '')
    {
        ...
        $user_info = $this->user->getInfo($condition, $field = 'uid,user_status,user_name,is_system,instance_id, is_member, current_login_ip, current_login_time, current_login_type,is_promoters');
        if (empty($user_info)) {
            if(empty($password)){
                $condition = array(
                    'user_tel' => $user_name
     ...
     */
    public function add($user_name, $password, $email, $mobile, $is_system, $qq_openid, $qq_info, $wx_openid, $wx_info,$wx_unionid, $is_member,$pid, $instance_id = 0)
    {
        if (! empty($user_name)) {
