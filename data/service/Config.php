...
class Config extends BaseService implements IConfig
{
    ...
    public function getUploadType($shop_id)
    {
        ...
        if (empty($upload_type)) {
            $res = $this->addConfig($shop_id, "UPLOAD_TYPE", 1, "上传方式 1 本地  2 xx", 1);
            return 1;
        } else {
    ...
    public function getQiniuConfig($shop_id)
    {
        ...
        if (empty($qiniu_info)) {
            ...
            $res = $this->addConfig($shop_id, "QINIU_CONFIG", json_encode($data), "xx云存储参数配置", 1);
            if (! $res > 0) {
                return null;
            } else {
                $qiniu_info = $this->config_module->getInfo([
    ...
    public function setUploadType($shop_id, $value)
    {
        ...
        if (! empty($upload_info)) {
            ...
            $res = $this->addConfig($shop_id, "UPLOAD_TYPE", $value, "上传方式 1 本地  2 xx", 1);
        }
        // TODO Auto-generated method stub
    ...
    */
    function getShortcutMenu($shop_id,$uid){
        $model = new SysShortcutMenuModel();
        $condition = ['shop_id'=>$shop_id,'uid'=>$uid];
