...
class Course extends BaseService implements ICourse
{
            ...
            if ($goods_id == 0) {
                ...
                if (! empty($sku_array)) {
                    ...
                    foreach ($sku_list_array as $k => $v) {
                        $res = $this->addOrUpdateGoodsSkuItem($this->course->goods_id, $v);
                        if (! $res) {
                            $error = 1;
                        }
                    }
    ...
    public function deleteGoods($goods_id)
    {
        ...
        Cache::tag("niu_course_category_block")->clear();
    ...
    public function getBackStageGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc')
    {
        ...
        $viewObj = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
            ->field("ng.goods_id,ng.goods_name,ng.promotion_price,ng.market_price,ng.goods_type,ng.stock,ng.introduction,ng.max_buy,ng.state,ng.is_hot,ng.is_recommend,ng.is_new,ng.sales,ng_sap.pic_cover_micro,ng.code,ng.create_time,ng.QRcode,ng.price,ng.real_sales,ng.sort,ng.group_id_array,ng.goods_kind,ng.allow_delete");
        $queryList = $goods_model->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
        $queryCount = $this->getGoodsQueryCount($condition);
        $list = $goods_model->setReturnList($queryList, $queryCount, $page_size);
