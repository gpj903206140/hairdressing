...
class Member extends User implements IMember
{
     ...
     */
    public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid,$pid)
    {
        // if (! empty($user_name)) {
        ...
        $res = parent::add($user_name, $password, $email, $mobile, 0, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, 1,$pid);
        if ($res > 0) {
            // 获取默认会员等级id
     ...
     * @see \data\api\niufenxiao\INfxUser::addMemberBalanceWithdraw()
     */
    public function addDistributionBalanceWithdraw($shop_id, $withdraw_no, $uid, $bank_account_id, $cash)
    {
        // 得到本店的提线设置
        $config = new Config();
        $withdraw_info = $config->getBalanceWithdrawConfig($shop_id);
        ...
        $user = new UserModel();
        $member_account = new MemberAccount();
        $commission = $user->getInfo(['uid'=>$uid],'can_commission');
        if ($commission['can_commission'] <= 0) {
            return DISTRIBUTION_ZEROERROR;
        }
        if ($commission['can_commission'] < $cash || $cash <= 0) {
            return DISTRIBUTION_LESSMONEYERROR;
        }
        // 获取 提现账户
        $member_bank_account = new NsMemberBankAccountModel();
        $bank_account_info = $member_bank_account->getInfo([
        ...
        $balance_withdraw->save($data);
        // 添加佣金账户流水
        $member_account->addDistributionAccountData($shop_id, 4, $uid, 0, - $cash, 10, $balance_withdraw->id, "分销佣金提现");
        if ($balance_withdraw->id) {
            $data['id'] = $balance_withdraw->id;
            hook("DistributionWithdrawApplyCreateSuccess", $data);
        }
        return $balance_withdraw->id;
    }
    /*
    ...
    /*佣金提现审核
     */
    public function DistributionBalanceWithdrawAudit($shop_id, $id, $status, $transfer_type, $transfer_name, $transfer_money, $transfer_remark, $transfer_no, $transfer_account_no, $type_id)
    {
        ...
        if ($transfer_status != - 1) {
            ...
            if ($retval > 0 && $status == - 1) {
                $member_account->addDistributionAccountData($shop_id, 4, $member_balance_withdraw_info['uid'], 1, $member_balance_withdraw_info["cash"], 11, $id, "分销佣金提现退回");
            }else{
                $user = new UserModel();
                $user->where(['uid' => $member_balance_withdraw_info['uid']])->setInc('stop_commission',$member_balance_withdraw_info["cash"]);
            }
            if ($retval > 0 && $status == 1) {
                // 会员提现审核通过钩子
                hook('DistributionWithdrawAuditAgree', [
                    'id' => $id
                ]);
            }
            return array(
     ...
     * @see \data\api\niufenxiao\INfxUser::MemberBalanceWithdrawRefuse()
     */
    public function userDistributionCommissionWithdrawRefuse($shop_id, $id, $status, $remark)
    {
        // TODO Auto-generated method stub
        $member_balance_withdraw = new NsMemberBalanceWithdrawModel();
        $member_account = new MemberAccount();
     ...
     * (non-PHPdoc)
     *
     * @see IMember::getMemberExtractionBalanceList()
     */
