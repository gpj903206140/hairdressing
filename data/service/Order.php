...
use data\model\NsOrderContinuedMoneyModel as ContinuedMoney;
use think\Session;
...
class Order extends BaseService implements IOrder
{
     ...
     * 收银列表
     * (non-PHPdoc)
     * @see \data\api\order::collectMoneyList()
     */
    public function collectMoneyList($page_index = 1, $page_size = 0, $condition = '', $order = 'create_time asc', $field = '*')
    { 
        $CollectMoney = new CollectMoney();
        $list = $CollectMoney->pageQuery($page_index, $page_size, $condition, $order, $field);
        return $list;
    }
    /**
     * 收银金额添加
     *
     * @param unknown $collect_id            
     * @param unknown $money            
     * @param unknown $remark                      
     * @param unknown $create_time                      
     */
    public function addContinuedMoney($collect_id, $money, $remark, $create_time)
    {
        $ContinuedMoney = new ContinuedMoney();
        $CollectMoney = new CollectMoney();
        $ContinuedMoney->startTrans();
        $data = array(
        ...
        //添加收银金额
        $result1 = $ContinuedMoney->save($data);
        $result2 = $CollectMoney->where('collect_id',$collect_id)->setInc('realPayment_money',$money);
        if($result1&&$result2){
            $retval['code'] = 1;
            $retval['message'] = '金额添加成功';
            $CollectMoney->commit();
        }else{
            $retval['code'] = 0;
            $retval['message'] = '金额添加失败';
            $CollectMoney->rollback();
        }
        return $retval;
    }
    /**
     ...
     * @param unknown $meet_money            
     * @param unknown $coupon_name           
     * @param unknown $coupon_id  
     * @param unknown $coupon_money            
     * @param unknown $at_leasts            
     * @param unknown $realPayment_money           
     * @param unknown $payment_method           
     * @param unknown $accountsWx_money           
     * @param unknown $paymentDate            
     * @param unknown $create_time                      
     */
    public function updateCollectMoney($collect_id, $order_id, $goods_id, $goods_name, $reserve_phone, $order_no, $uid, $nick_name, $money, $meet_money, $coupon_name, $coupon_id, $coupon_money, $at_leasts, $realPayment_money, $payment_method, $accountsWx_money, $paymentDate, $remark, $create_time)
    {
        $CollectMoney = new CollectMoney();
        ...
        $count = $CollectMoney->getCount(['order_no'=>$order_no]);
        $orderId = $order_model->getInfo(['order_no'=>$order_no,'order_status'=>10],'order_id');

        if($count>0){
            return 2;
        }
        //添加收银
        $result1 = $CollectMoney->save($data);
        if(!$result1){
            $retval = 0;
            $CollectMoney->rollback();
            return $retval;
        ...
        if(!empty($coupon_id)){
            $row['state'] = 2;
            $row['use_time'] = time();
            $result2 = $coupon->save($row,['coupon_id'=>$coupon_id]);
            if(!$result2){
                $retval = 0;
                $CollectMoney->rollback();
                return $retval;
            }
            if($meet_money<=$coupon_money){
                $coupon_money = $meet_money;
            }
            $orderArr['coupon_money'] = $coupon_money;
            $orderArr['coupon_id'] = $coupon_id;
        }
        //改变订单状态
        $result3 = true;
        if(!empty($orderId)){
            $orderArr['pay_status'] = 10;
            $orderArr['finish_time'] = time();
            $orderArr['order_money'] = $realPayment_money;
            $orderArr['goods_money'] = $meet_money;
            $orderArr['pay_money'] = $realPayment_money;
            $orderArr['pay_money'] = $realPayment_money;
            $result3 = $order_model->save($orderArr,['order_no'=>$order_no]);
            if(!$result3){
                $retval = 0;
                $CollectMoney->rollback();
                return $retval;
        ...
        $result4 = true;
        if(!empty($orderId)){
            $goodOrderArr['order_status'] = 10;
            $goodOrderArr['price'] = $meet_money;
            $goodOrderArr['goods_money'] = $realPayment_money;
            $goodOrderArr['order_status'] = 10;
            $result4 = $orderGoods->save($goodOrderArr,['order_id'=>$orderId['order_id'],'buyer_id'=>$uid]); 
            if(!$result4){
                $retval = 0;
                $CollectMoney->rollback();
                return $retval;
        ...
        //查找等级信息满足条件就修改
        $memberLevel = $member->getInfo(['uid'=>$uid],'member_level');
        //$level2 = $member_level->getInfo(['level_id'=>$memberLevel['member_level']],'level_id');
        //修改等级
        $orderSum1 = $order_model->getSum(['buyer_id'=>$uid,'order_status'=>10,'pay_status'=>10],'pay_money');
        $orderSum2 = $order_model->getSum(['buyer_id'=>$uid,'order_status'=>4],'pay_money');
        $pointSum = $order_model->getSum(['buyer_id'=>$uid,'order_status'=>4],'point');
        $orderSum = $orderSum1+$orderSum2;
        $total_money = $orderSum+$meet_money;
        $level = $member_level->getQuery([],'*','level_id desc');
        foreach($level as $l){
            if($l['relation']==1){
                if($total_money>=$l['quota']||$pointSum>=$l['min_integral']){
                    $level_id[] = $l['level_id'];
                }
        ...
        $level_id = max($level_id);
        if($memberLevel['member_level']<$level_id){
            $member->ModifyTableField('uid',$uid,'member_level',$level_id);
        }
        //发放线下订单佣金
        $result5 = true;
        $result6 = true;
        if(!empty($orderId['order_id'])){
            $commission_money = 0;
            $do = $DistributionOrder->getQuery(['order_id'=>$orderId['order_id'],'buyer_id'=>$uid],'*','add_time desc');
            if(!empty($do)){
                foreach($do as $val){
                    $res['dorder_id'] = $val['dorder_id'];
                    $res['price'] = $meet_money;
                    $res['profit'] = $meet_money;
                    $commission_money += $res['commission_money'] = $res['profit']*($val['distribution_commission_rate']/100)*($val['commission_rate']/100);
                    $res['is_issue'] = 5;
                    $res2[] = $res;
                    /*$result5 = $DistributionOrder->save($res,['dorder_id'=>$val['dorder_id']]);
                    if(!$result5){
                        $retval = 0;
                        $CollectMoney->rollback();
                        return $retval;
                ...
                $result5 = $DistributionOrder->saveAll($res2);
                if(!$result5){
                    $retval = 0;
                    $CollectMoney->rollback();
                    return $retval;
                }
                $result6 = $this->updateCommissionDistributionGrant($orderId['order_id']);
                if(!$result6){
                    $retval = 0;
                    $CollectMoney->rollback();
                    return $retval;
            ...
            $commission['commission'] = $commission_money;
            $result7 = $order_model->save($commission,['order_id'=>$orderId['order_id']]);
            if(!$result7){
                $retval = 0;
                $CollectMoney->rollback();
                return $retval;
     ...
     * 预约订单创建
     * (non-PHPdoc)
     * @see \data\api\IOrder::orderCreateReservation()
     */
    public function orderCreateReservation($out_trade_no, $goods_sku_list, $use_coupon, $shipping_time, $num, $receiver_mobile, $buyer_message)
    {
        $order = new OrderBusiness();
        $retval = $order->orderCreateReservation($out_trade_no, $goods_sku_list, $use_coupon, $shipping_time, $num, $receiver_mobile, $buyer_message);
        runhook("Notify", "orderCreate", array(
            "order_id" => $retval
        ));
        // 针对特殊订单执行支付处理
    ...
    function updateCommissionDistributionIssue($order_id){
        ...
        if($set['is_open']==1){
            ...
            $order_model = new NsOrderModel();
            $orderList = $order_goods->getQuery(['order_id'=>$order_id],'*','order_goods_id desc');
            $i = 0;
            $orderCommission = 0.00;
            if(!empty($orderList)){
                foreach($orderList as $k=>$v){
                    $goodsCon = $goods->getInfo(['goods_id'=>$v['goods_id']],'distribution_commission_rate,is_open');
                    if($goodsCon['is_open']==1){
                        if($set['distribution_use']==0){ //使用利润  
                            $data[$i]['profit'] = ($v['price']-$v['cost_price'])*$v['num'];
                            if($data[$i]['profit']<=0){
                                $data[$i]['profit'] = 0;
                            }
                        }else if($set['distribution_use']==1){ //销售价格  
                            $data[$i]['profit'] = $v['goods_money'];
                        }
                        $data[$i]['add_time'] = time();
                        //商品买价
                        $data[$i]['price'] = $v['price'];
                        //商品商品成本价
                        $data[$i]['cost_price'] = $v['cost_price'];
                        $data[$i]['order_id'] = $order_id;
                        $data[$i]['goods_id'] = $v['goods_id'];
                        //购买人ID
                        $data[$i]['buyer_id'] = $v['buyer_id']; 
                        //商品分销比例
                        $data[$i]['distribution_commission_rate'] = $goodsCon['distribution_commission_rate'];
                        //受益人Id
                        $data[$i]['receiptor_id'] = $v['buyer_id'];
                        //获取上级推广员和分销级别
                        $userCon = $user->getInfo(['uid'=>$v['buyer_id']],'Pid,level_id,is_promoters');
                        $RromotersCon = $RankRromoters->getInfo(['level_id'=>$userCon['level_id']],'level_0,level_1');
                        //推广员佣金比例
                        $data[$i]['commission_rate'] = $RromotersCon['level_0'];
                        //佣金金额
                        $orderCommission += $data[$i]['commission_money'] = $data[$i]['profit']*($data[$i]['distribution_commission_rate']/100)*($RromotersCon['level_0']/100);
                        //上级分销佣金
                        $Relations = new Relations();
                        $relationList = $Relations->getQuery(['ChildId'=>$v['buyer_id']],'Pid','FxLevel desc');
                        $j = 1;
                        if(!empty($relationList)){
                            foreach($relationList as $p){
                                if(LEVEL>$j){
                                    $is_promoters = $user->getInfo(['uid'=>$p['Pid']],'level_id,is_promoters');
                                    if($is_promoters['is_promoters']==1){
                                        if($userCon['is_promoters']!=1){
                                            $pRromotersCon = $RankRromoters->getInfo(['level_id'=>$is_promoters['level_id']],'level_0,level_1');
                                            $RromotersLevel = $pRromotersCon['level_0'];
                                        }else{
                                            $RromotersLevel = $RromotersCon['level_'.$j];
                                        }
                                        $i++;
                                        $data[$i] = $data[$i-1];
                                        $data[$i]['add_time'] = time();
                                        $data[$i]['receiptor_id'] = $p['Pid'];
                                        $data[$i]['commission_rate'] = $RromotersLevel;
                                        $orderCommission += $data[$i]['commission_money'] = $data[$i]['profit']*($data[$i]['distribution_commission_rate']/100)*($RromotersLevel/100);
                                    }
                                    $j++;
                                }
                            }
                ...
                if(!empty($data)){
                    $DistributionOrder->startTrans();
                    $reslut1 = $DistributionOrder->saveAll($data);
                    $reslut2 = $order_model->ModifyTableField('order_id',$order_id,'commission',$orderCommission);
                    if(!empty($reslut1)&&$reslut2||$reslut1&&$reslut2){
                        $DistributionOrder->commit();
                        return true;
                    }else{
                        $DistributionOrder->rollback();
                        return false;
                    }
                }
    ...
    发放订单的分销佣金
     */
    function updateCommissionDistributionGrant($order_id){
        //$order_goods = new NsOrderGoodsModel();
        //$goods = new NsGoodsModel();
        $user = new UserModel();
        $DistributionOrder = new DistributionOrder();
        //$RankRromoters = new NsRankRromoters();
        $row = $DistributionOrder->getQuery(['order_id'=>$order_id],'sum(commission_money) as money,receiptor_id','dorder_id desc','receiptor_id');
        $i = 0;
        $user->startTrans();
        $count = count($row);
        foreach($row as $k=>$v){
            $money = $user->getInfo(['uid'=>$v['receiptor_id']],'total_commission,can_commission','dorder_id desc');
            $commission['total_commission'] = $money['total_commission']+$v['money'];
            $commission['can_commission'] = $money['can_commission']+$v['money'];
            if($user->save($commission,['uid'=>$v['receiptor_id']])){
                $i++;
            }
        }
        $data['is_issue'] = 1;
        $data['text'] = '订单交易完成，二级分销佣金进行结算。';
        $reslut = $DistributionOrder->save($data,['order_id'=>$order_id]);
        if($count==$i&&$reslut){
            $user->commit();
            return true;
        }else{
    ...
    退款返还佣金
     */
    function updateCommissionDistributionRestore($order_id){
        $user = new UserModel();
        $DistributionOrder = new DistributionOrder();
        $row = $DistributionOrder->getQuery(['order_id'=>$order_id],'sum(commission_money) as money,receiptor_id','dorder_id desc','receiptor_id');
        $i = 0;
    ...
    通过销售总额判断是否升级推广员等级
     */
    function updateCommissionDistributionGrade(){
        $fileName = INTERCALATE;
        $set = array();
        $config_model = new ConfigModel();
        $thisconfig = $config_model->getInfo(['key'=>'CONFIG_DISTRIBUTION'],'value');
        ...
        if($set['is_open']==1){
            ...
            $userCon = $user->getInfo(['uid'=>$uid],'level_id,is_promoters');
            if($userCon['is_promoters']){
               $totalMoney = $order_model->getSum(['buyer_id'=>$uid,'order_status'=>['eq',4]],'order_money');
               $NsRankRromoters = new NsRankRromoters();
               $level = $NsRankRromoters->getFirstData(['level_money'=>['lt',$totalMoney]],'level_id desc');
               $row['level_id'] = $level['level_id'];
               $row['level_name'] = $level['level_name'];
               if($userCon['level_id']<$level['level_id']){
                  $user->save($row,['uid'=>$uid]);
               }
     ...
     * 查询预约商品总库存是否为小于或等于0
     */
    public function getGoodsStock($goods_sku_list)
    {
        $array = array();
        $res_array = array();
        $messages = "";
        if (! empty($goods_sku_list)) {
            $ns_goods_model = new NsGoodsModel();
                $goods = $ns_goods_model->getInfo([
                    'shop_id' => $this->instance_id,
                    "goods_id" => $goods_sku_list
                ], "stock,goods_name");
        }
        if ($goods['stock'] <= 0) {
            $messages = "您预约的商品“" . $goods['goods_name'] . "”已经没有了";
        }
        return $messages;
    }
     ...
     * @see \data\api\IOrder::orderGoodsRefundAskfor()
     */
    public function updateCancelReservation($order_id, $order_goods_id, $refund_reason)
    {
        $order_goods = new OrderGoods();
        $retval = $order_goods->updateCancelReservation($order_id, $order_goods_id, $refund_reason);
        if ($retval) {
            $params = [
                'order_id' => $order_id,
                'order_goods_id' => $order_goods_id,
            ...
            runhook("Notify", "updateCancelReservationBusiness", [
                "shop_id" => 0,
                "order_id" => $order_id
            ]); // 商家退款提醒
            hook('updateCancelReservationSuccess', $params);
        }
        return $retval;
        // TODO Auto-generated method stub
    }
        ...
        //if (NS_VERSION != NS_VER_B2C) {
            ...
            try {
                ...
                if ($order_sataus == ORDER_COMPLETE_SUCCESS || $order_sataus == ORDER_COMPLETE_REFUND || $order_sataus == ORDER_COMPLETE_SHUTDOWN) {
                    ...
                    //发放订单的分销佣金
                    $this->updateCommissionDistributionGrant($order_id);
                    //通过销售总额判断是否升级推广员等级
                    $this->updateCommissionDistributionGrade();
    ...
    public function orderCustomerConfirmRefund($id, $order_id, $order_goods_id, $refund_real_money, $refund_balance_money, $refund_way, $refund_remark)
    {
        ...
        if ($order_refund_chai < $refund_real_money_ext) {
            ...
            if ($refund['is_success'] == 1) {
                ...
                //退款返还佣金
                $this->updateCommissionDistributionRestore($order_id);
                // 重新计算订单的佣金情况
               // $this->updateCommissionMoney($order_id, $order_goods_id);
