...
class OrderGoods extends BaseService
{
    ...
    {
        $course_order_goods = new NsOrderGoodsModel();
        ...
        try {
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $order_goods = new NsCourseOrderGoodsModel();
                $order_goods->save($data_order_sku);
            }
            if ($err == 0) {
     ...
     * @param unknown $goods_sku_list            
     */
    public function addOrderGoodsReservation($order_id, $goods_sku_list, $num, $adjust_money = 0)
    {
        $this->order_goods->startTrans();
        try {
            $err = 0;
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $goods_calculate = new GoodsCalculate();
                /*$goods_calculate->subGoodsStock($goods_sku_info['goods_id'], $goods_sku_info['sku_id'], $goods_sku[1], '');*/
                $goods_calculate->addGoodsSales($goods_sku_array, 0, 1);

                $goods1 = $goods_model->where(['goods_id'=>$goods_sku_array])->setDec('stock',1);
                $goods2 = $goods_model->where(['goods_id'=>$goods_sku_array])->setInc('sales',1);
                if(!$goods1||!$goods2){
                    $err = 1;
                }
                $data_order_sku = array(
                    ...
                    'cost_price' => 0,
                    'goods_money' => 0,
                    'goods_picture' => $goods_info['picture'], // 如果当前商品有SKU图片，就用SKU图片。没有则用商品主图
                    'shop_id' => $this->instance_id,
                    'buyer_id' => $this->uid,
                    'goods_type' => $goods_info['goods_type'],
                    'promotion_id' => $goods_info['promote_id'],
    ...
    public function orderGoodsRefundAgree($order_id, $order_goods_id)
    {
        ...
        try {
            ...
            $orders = $order_model->getInfo(['order_id'=>$order_id],'order_status');
            ...
            if($orders['order_status']==10){
                $order_model->ModifyTableField('order_id',$order_id,'pay_status','-10');
            }
            $this->order_goods->commit();
            return 1;
        } catch (\Exception $e) {
    ...
    public function orderGoodsRefuseForever($order_id, $order_goods_id)
    {
        ...
        try {
            ...
            if($orders['order_status']!=10){
                ...
                $res = $order_model->ModifyTableField('order_id',$order_id,'pay_status','-13');
            }
    ...
    public function orderGoodsRefuseOnce($order_id, $order_goods_id)
    {
        ...
        try {
            ...
            if($orders['order_status']!=10){
                ...
                $res = $order_model->ModifyTableField('order_id',$order_id,'pay_status','-12');
            }
