...
class Order extends BaseService
{
    ...
    public function orderCreate($order_type, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, $coupon_id, $user_money, $goods_sku_list, $platform_money, $pick_up_id, $shipping_company_id, $coin, $fixed_telephone = "", $presell_money = 0)
    {
        ...
        try {
            ...
            $data_order = array(
                ...
                'order_no' => $this->createOrderNo($this->uid),
            ...
            if($buyer_info['is_promoters']==1){
                $res_orderService = $orderService->updateCommissionDistributionIssue($order_id);
                if (! ($res_order_goods > 0)&&!$res_orderService) {
                    $this->order->rollback();
     ...
     * @param unknown $use_coupon            
     * @param unknown $sign_time            
     * @param unknown $num            
     * @param unknown $receiver_mobile            
     * @param unknown $memo
     */
    public function orderCreateReservation($out_trade_no, $goods_sku_list, $coupon_id, $shipping_time, $num, $receiver_mobile, $buyer_message)
    {
        $this->order->startTrans();
        ...
        try {
            ...
            $data_order = array(
                ...
                'buyer_message' => $buyer_message,
                'buyer_invoice' => $buyer_invoice,
                'shipping_time' => strtotime($shipping_time), // datetime NOT NULL COMMENT '买家要求配送时间',
                'receiver_mobile' => $receiver_mobile, // varchar(11) NOT NULL DEFAULT '' COMMENT '收货人的手机号码',
                'receiver_province' => $receiver_province, // int(11) NOT NULL COMMENT 
                //默认值
                'receiver_city' => 0, // int(11) NOT NULL COMMENT 
                'receiver_district' => 0, // int(11) NOT NULL COMMENT 
                'point' => 0, // int(11) NOT NULL COMMENT 
                'coupon_money' => 0, // int(11) NOT NULL COMMENT
                'coupon_id' => 0, // int(11) NOT NULL COMMENT
                'promotion_money' => 0, // int(11) NOT NULL COMMENT
                'shipping_money' => 0, // int(11) NOT NULL COMMENT
                'give_point' => 0, // int(11) NOT NULL COMMENT

                'shop_id' => $shop_id, // int(11) NOT NULL COMMENT '卖家店铺id',
                'shop_name' => $shop_name, // varchar(100) NOT NULL DEFAULT '' COMMENT '卖家店铺名称',
                'goods_money' => 0, // decimal(19, 2) NOT NULL COMMENT '商品总价',
                'order_money' => 0, // decimal(10, 2) NOT NULL COMMENT '订单总价',
                'point_money' => 0, // decimal(10, 2) NOT NULL COMMENT 
            ...
            if ($coupon_id > 0) {
                ...
                $max_fetch = $CouponType->getInfo(['coupon_type_id'=>$coupon_id],'max_fetch');
                $couponCount = $Coupon->getCount(['coupon_type_id'=>$coupon_id,'state'=>['gt','0'],'uid'=>$this->uid]);
                if($max_fetch>0&&$couponCount<$max_fetch||$max_fetch==0){
                    $data_details = array(
                    ...
                    $couponid = $Coupon->getInfo(['coupon_type_id'=>$coupon_id,'state'=>0],'coupon_id');
                    $Coupon_state = $Coupon->save($data_details,['coupon_id'=>$couponid['coupon_id']]);
                }
                
            }
            // 添加订单项
            $order_goods = new OrderGoods();
            $res_order_goods = $order_goods->addOrderGoodsReservation($order_id, $goods_sku_list,$num);
    ...
    public function orderGoodsRefundFinish($order_id)
    {
        ...
        try {
            ...
            if ($refunding_count > 0) {
                $order_model = new NsOrderModel();
                $order = $order_model->getInfo(['order_id'=>$order_id],'order_status');
                if($order['order_status']!=10){
                    $orderInfo->order_status = OrderStatus::getOrderCommonStatus()[6]['status_id']; // 退款中
                }else{
                    $order_model->ModifyTableField('order_id',$order_id,'pay_status','-11');
                }
