<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:50:"template/adminblue\course\courseCatalogueList.html";i:1545977073;s:28:"template/adminblue\base.html";i:1545011442;s:45:"template/adminblue\controlCommonVariable.html";i:1530933188;s:32:"template/adminblue\urlModel.html";i:1531183408;s:34:"template/adminblue\pageCommon.html";i:1538099192;s:34:"template/adminblue\openDialog.html";i:1522669943;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<div class="options-btn">
	<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('ADMIN_MAIN/course/addCourseCatalogue','goods_id='.$goods_id); ?>';">添加课程目录</button>
</div>
...
<table class="table-class">
	...
	<tbody>
		...
		<tr class="pid_0" style="height: 30px;" data-category-id="<?php echo $v1['catalogue_id']; ?>" data-pid="" data-level="">
			<td>
				
			</td>
			
			<td>
				<?php echo $v1['catalogue_name']; ?>
				
			</td>
			<td></td>
			<td align="center">
			<a href="javascript:void(0);" onclick="showEditBg(<?php echo $v1['catalogue_id']; ?>);">查看</a>   
			<a href="<?php echo __URL('ADMIN_MAIN/course/addCourseCatalogueVideo','goods_id='.$goods_id.'&catalogue_id='.$v1['catalogue_id']); ?>">添加</a>
			</td>
			<td style="text-align: center;"><?php echo !empty($v1['is_visible'])?'是' : '否'; ?></td>
			<td style="text-align: center;"><input type="number" class="sort input-common input-common-sort" fieldid="<?php echo $v1['catalogue_id']; ?>" fieldname="sort" value="<?php echo $v1['sort']; ?>" size="1"></td>
			<td style="text-align: center;">
				<a href="<?php echo __URL('ADMIN_MAIN/course/updateCourseCatalogue','goods_id='.$goods_id.'&catalogue_id='.$v1['catalogue_id']); ?>">修改</a>
				<a href="javascript:void(0);" onclick="delCategory(<?php echo $goods_id; ?>,<?php echo $v1['catalogue_id']; ?>)">删除</a>
			</td>
		</tr>
...
<?php else: ?>
	<div class="tip-box">当前系统还没有课程目录，<a href="<?php echo __URL('ADMIN_MAIN/course/addCourseCatalogue','goods_id='.$goods_id); ?>">马上添加</a></div>
<?php endif; ?>
...
function showEditBg(promoter_id){
	$("#uid").val(promoter_id);
	$("#editBg").modal('show');
	var url="<?php echo __URL('ADMIN_MAIN/course/CourseCatalogueVideo'); ?>&id="+promoter_id;
...
function delEditBg(pic_id,obj){
	var url="<?php echo __URL('ADMIN_MAIN/course/delCourseCatalogueVideo'); ?>&id="+pic_id;
	$.get(url,function(data){
	   if(data.code==1){
	   	  $(obj).parent().remove();
	   	  layer.msg('删除课程视频成功');
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/course/deleteCourseCatalogue'); ?>",
					data : {
						'goods_id' : goods_id,
					...
					success : function(data) {
						if(data['code'] > 0){
							...
							location.href = "<?php echo __URL('ADMIN_MAIN/course/CourseCataloguelist'); ?>&goods_id=<?php echo $goods_id; ?>";
						}else{
...
$("#saveNewAddedCategory").live('click', function(event) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/course/batchAddCourseCatalogue'); ?>",
		data : { "content" : content },
		success : function(data){
			if(data["code"] > 0){
				showTip(data['message'],"success");
				location.href = "<?php echo __URL('ADMIN_MAIN/course/CourseCataloguelist'); ?>";
			}else{
				showTip(data['message'],"error");
				location.href = "<?php echo __URL('ADMIN_MAIN/course/CourseCataloguelist'); ?>";
			}
...
function save(){
	...
	if(verify( category_name,short_name, sort)){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('ADMIN_MAIN/course/addCoursegoods'); ?>",
			data : {
