<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:61:"template/adminblue\Distributionnew\stayPromotersTubeList.html";i:1540866431;s:28:"template/adminblue\base.html";i:1546935702;s:45:"template/adminblue\controlCommonVariable.html";i:1530933188;s:32:"template/adminblue\urlModel.html";i:1531183408;s:34:"template/adminblue\pageCommon.html";i:1538099192;s:34:"template/adminblue\openDialog.html";i:1522669943;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<nav class="ns-third-menu" style="margin:0">
	<ul>
		<li class="<?php if($state == 0): ?>selected<?php endif; ?>" onclick='location.href=__URL("ADMIN_MAIN/distributionnew/stayPromotersTubeList&state=0")'>推广员</li>
		<li class="<?php if($state == 1): ?>selected<?php endif; ?>" onclick='location.href=__URL("ADMIN_MAIN/distributionnew/stayPromotersTubeList&state=1")'>待审核</li>
	</ul>
</nav>
<div class="ns-main">
            ...
            <div class="modal-body">
            	<div class="modal-div">
            		...
            		<dl class="modal-dl">
            			...
            			<dd>
            				<select id="level_id">
            				    <?php if(is_array($RromotersList) || $RromotersList instanceof \think\Collection || $RromotersList instanceof \think\Paginator): $i = 0; $__LIST__ = $RromotersList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
...
function searchData(){
	...
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersTubeList'); ?>&state="+state,
			async : true,
			data : {
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						if(data["data"][i]["is_promoters"] == 1){
				            ...
				            html += '<a href="javascript:void(0);" onclick="showEditLevel(' + data["data"][i]["uid"] + ',' + data["data"][i]["level_id"] + ');">修改等级</a> <a href="javascript:void(0);" onclick="showTeamList(' + data["data"][i]["uid"] + ');">我的团队</a>';
	...
	function audit(is_promoters, uid){
		$.ajax({
			type : "post",
			url : "<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersAudit'); ?>",
			async : true,
			data : {
				"is_promoters" : is_promoters,
				"uid" : uid
			...
			success : function(data) {
				if(is_promoters==1){
					var name='已通过';
					var href="<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersTubeList'); ?>";
				}else{
					var name='已拒绝';
					var href="<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersTubeList'); ?>&state=1";
				}
				if(data.code == 1){
					showMessage('success', '推广员申请'+name+'成功',href);
					//location.reload();
				}else{
					showMessage('error', '推广员申请'+name+'失败',"<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersTubeList'); ?>&state=1");
					//location.reload();
				}
			}
    	})
	...
	function deletePromoter(uid){
		$.ajax({
			type:"post",
			url:"<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersDelete'); ?>",
			data:{
				'uid':uid
			},
			async:true,
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersTubeList'); ?>&state=<?php echo $state; ?>");
				}else{
	...
	function modifyPromoterParent(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('ADMIN_MAIN/distributionnew/updateSuperior'); ?>",
			data:{
			...
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/distributionnew/stayPromotersTubeList&state=0'); ?>");
	...
	function updatePromoterLevel(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('ADMIN_MAIN/distributionnew/updatePromoterLevel'); ?>",
			data:{
				'promoter_id':promoter_id,
				'level_id':level_id
			},
	...
	function showTeamList(uid){
		$.ajax({
			type:"post",
			url:"<?php echo __URL('ADMIN_MAIN/distributionnew/teamList'); ?>",
			data:{
				'uid':uid
			},
			async:true,
