<?php if (!defined('THINK_PATH')) exit(); /*a:7:{s:50:"template/adminblue\course\courseMechanismList.html";i:1545282846;s:28:"template/adminblue\base.html";i:1545011442;s:45:"template/adminblue\controlCommonVariable.html";i:1530933188;s:32:"template/adminblue\urlModel.html";i:1531183408;s:55:"template/adminblue\System\uploadCourseMechanismImg.html";i:1545037570;s:34:"template/adminblue\pageCommon.html";i:1538099192;s:34:"template/adminblue\openDialog.html";i:1522669943;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
	...
	$('#fileupload').fileupload({
		...
		add: function (e,data) {
			...
			if(param.state > 0){
				...
				var img='<span><img src="'+param.file_name+'"><a onclick="delEditBg('+param.file_id+',this);"></a></span>';
				dd.append(img);
			} else {
...
<div class="options-btn">
	<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('ADMIN_MAIN/course/addCourseMechanism'); ?>';">添加合作机构</button>
</div>
...
<table class="table-class">
	...
	<tbody>
		...
		<tr class="pid_0" style="height: 30px;" data-category-id="<?php echo $v1['mechanism_id']; ?>" data-pid="" data-level="">
			<td>
				
			</td>
			
			<td>
				<?php echo $v1['mechanism_name']; ?>
				
			</td>
			<td><a href="<?php echo __URL('ADMIN_MAIN/course/courseTeacherList','mechanism_id='.$v1['mechanism_id']); ?>">查看</a></td>
			<td align="center"><?php echo $v1['teacher_num']; ?></td>
			<td align="center"><a href="javascript:void(0);" onclick="showEditBg(<?php echo $v1['mechanism_id']; ?>);">查看</a></td>
			<td style="text-align: center;"><?php echo !empty($v1['is_visible'])?'是' : '否'; ?></td>
			<td style="text-align: center;"><input type="number" class="sort input-common input-common-sort" fieldid="<?php echo $v1['mechanism_id']; ?>" fieldname="sort" value="<?php echo $v1['sort']; ?>" size="1"></td>
			
			<td style="text-align: center;">
				<a href="<?php echo __URL('ADMIN_MAIN/course/updateCourseMechanism','mechanism_id='.$v1['mechanism_id']); ?>">修改</a>
				<a href="javascript:void(0);" onclick="delCategory(<?php echo $v1['mechanism_id']; ?>)">删除</a>
			</td>
		</tr>
...
<?php else: ?>
	<div class="tip-box">当前系统还没有合作机构，<a href="<?php echo __URL('ADMIN_MAIN/course/addCourseMechanism'); ?>">马上添加</a></div>
<?php endif; ?>
...
function showEditBg(promoter_id){
	$("#uid").val(promoter_id);
	$("#editBg").modal('show');
	var url="<?php echo __URL('APP_MAIN/upload/setSessionId'); ?>&id="+promoter_id;
...
function delEditBg(pic_id,obj){
	var url="<?php echo __URL('APP_MAIN/upload/delUimg'); ?>&id="+pic_id;
	$.get(url,function(data){
	   if(data==1){
	   	  $(obj).parent().remove();
	   	  layer.msg('删除照片墙图片成功');
...
$(".table-class tr[data-category-id] input").live("change",function(){
	...
	if(fieldvalue == ''){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('ADMIN_MAIN/course/modifyCourseMechanismField'); ?>",
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/course/deleteCourseMechanism'); ?>",
					data : {
					...
					success : function(data) {
						if(data['code'] > 0){
							...
							location.href = "<?php echo __URL('ADMIN_MAIN/course/coursemechanismlist'); ?>";
						}else{
...
$("#saveNewAddedCategory").live('click', function(event) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/course/batchAddcoursemechanism'); ?>",
...
function save(){
	...
	if(verify( category_name,short_name, sort)){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('ADMIN_MAIN/course/addCourseMechanism'); ?>",
			data : {
