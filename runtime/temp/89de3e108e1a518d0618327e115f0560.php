...
<table class="mytable">
	<tr>
		<th align="left">
			<a class="btn-common" href="<?php echo __URL('ADMIN_MAIN/course/addCourse'); ?>">发布课程</a>
			...
			data-content="<div class='edit-group' id='recommendType'>
				...
				"
			>推荐</a>
			<a data-html="true" class="btn-common-white fun-a category" href="javascript:goodsGroupIdCount();" id="editGroup" title="修改课程标签" data-container="body" data-placement="bottom"  data-trigger="manual"
...
<input type="text" placeholder="请选择课程分类" id="goodsCategoryOne" is_show="false" class="input-common middle">
...
$(".js-category-one").click(function(){
	...
	$.ajax({
		type : 'post',
		url : "<?php echo __URL('ADMIN_MAIN/course/getcategorybyparentajax'); ?>",
...
$("#confirmSelect").click(function(){
	...
	$(".selectGoodsCategory").css({
		...
		'right' : 580
			...
			<span>课程名称：</span>
			<input id="goods_name" class="input-medium input-common middle" type="text" value="<?php echo $search_info; ?>" placeholder="要搜索的课程名称" >	
			<span class="interval"></span>
			...
			<div class="more-search-container">
				<dl>
					<dt>课程标签：</dt>
					<dd>
						<input type="text" placeholder="请选择课程标签" id="selectGoodsLabel"  onfocus="selectGoodsLabel();" is_show="false" data-html="true" class="input-common middle" title="<h6 class='edit-group-title'>选择课程标签</h6>" data-container="body" data-placement="bottom"  data-trigger="manual" data-content="<div class='edit-group' style='max-width:auto;'>
							<?php foreach($goods_group as $vo): ?>
							<p>
...
<!-- 					<th>上下架</th> -->
				...
				<!-- <tr>
					<td colspan="9" style="padding:10px 6px;border-left:1px solid #E1E6F0;border-right:1px solid #E1E6F0;">
						<a class="btn-common-child" href="<?php echo __URL('ADMIN_MAIN/goods/addgoods'); ?>">+发布课程</a>
						...
						data-content="<div class='edit-group' id='recommendType'>
							...
							"
						>推荐</a>
						<a data-html="true" class="btn-common-child fun-a category" href="javascript:goodsGroupIdCount();" id="editGroup" title="修改课程标签" data-container="body" data-placement="bottom"  data-trigger="manual"
							data-content="<div class='edit-group' id='goodsChecked' style='max-width:auto;'>
 							<?php foreach($goods_group as $vo): ?>
 							<p>
 							<label class='checkbox-inline' style='display:inline-block;' >
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/course/courselist'); ?>",
		data : {
...
// 							}
						...
						html += '<td style="text-align: right;">';
							html += '<div class="cell">';
								html += '<span class="pro-stock" style="color: #666;" id="moreChangeStock'+ data["data"][i]["goods_id"] + '">' + data["data"][i]["total_num"] + '</span>';
							html += '</div>';
						html += '</td>';
						html += '<td style="text-align: right;">';
							html += '<div class="cell">';
								html += '<span class="pro-stock" style="color: #666;" id="moreChangeStock'+ data["data"][i]["goods_id"] + '">' + data["data"][i]["release_num"] + '</span>';
							html += '</div>';
						html += '</td>';
						...
						html += '<td style="text-align: center;">'+ data["data"][i]["crowd"]+'</td>';
                        html += '<td style="text-align: center;"><a href="' + __URL("ADMIN_MAIN/course/courseCatalogueList?step=2&goods_id="+ data["data"][i]["goods_id"]) + '" title="查看课程目录" >查看</a></td>';
						html += '<td style="text-align: center;">';
							html += '<div class="cell">';
								html += '<input class="input-common input-common-sort" goods_id="' + data["data"][i]["goods_id"] + '"  value="' + data["data"][i]["sort"] + '" onchange="changeSort(this)"' + 'type="number" title="排序号数值越大，商城课程列表显示越靠前">';
							html += '</div>';
						html += '</td>';

						html += '<td>';
							html += '<div class="bs-docs-example tooltip-demo"style="text-align: center;">';
								html += '<a href="' + __URL("ADMIN_MAIN/course/addCourse?step=2&goodsId="+ data["data"][i]["goods_id"]) + '" title="编辑课程" >编辑</a>';
								html += '<a href="javascript:copyGoodsDetail(' + data["data"][i]["goods_id"] + ');" title="复制课程" >复制</a>';
								if(data["data"][i]["allow_delete"]==1){
...
function modifyGoodsOnline(goods_ids,line){
	if(line == "online"){
		var url = "<?php echo __URL('ADMIN_MAIN/Course/modifycorseonline'); ?>";
		var lingStr = "上架";
	}else{
		var url = "<?php echo __URL('ADMIN_MAIN/Course/modifycorseoffline'); ?>";
		var lingStr = "下架";
	}
	$.ajax({
		type : "post",
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/course/deletecourse'); ?>",
...
function goodsGroupUpdate(){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/course/modifygoodsgroup'); ?>",
...
function setRecommend(){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/course/modifygoodsrecommend'); ?>",
		data : {
			"goods_id" : goods_ids,
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/course/copycourse'); ?>",
