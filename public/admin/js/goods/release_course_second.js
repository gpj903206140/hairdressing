...
//		$('#presell_day').parents('dl').addClass('hide');
	...
	$('#img_box').live('click',function(e){
		...
		shopImageFlag = js_img;//所点击的课程图片标识
...
function ValidateUserInput() {
	...
	if (!IsEmpty("#txtProductTitle")) {
		...
		$("#txtProductTitle").next("span").text("请填写课程名称").show();
		$("#txtProductTitle").focus();
		return false;
	}else if($("#txtProductTitle").val().length>60){
		$(".goods-nav ul li:eq(0)").click();
		$("#txtProductTitle").next("span").text("课程标题不能大于60个字").show();
		$("#txtProductTitle").focus();
		return false;
	} else {
	...
	if($(".upload_img_id").length == 0){
		...
		$(".img-error").text("最少需要一张图片作为课程主图").show();
		return false;
	}else{
		$(".img-error").hide();
	}
	...
	if (description == "") {
		...
		showTip("课程描述不能为空","warning");
		$("#tareaProductDiscrip").nextAll("span:last").text("课程描述不能为空").show();
		$("body").scrollTop($("#discripContainer").offset().top-100);
		return false;
	} else if (description.length < 5 || description.length > 25000) {
		$(".goods-nav ul li:eq(4)").click();
		showTip("课程描述字符数应在5～25000之间","warning");
		$("#tareaProductDiscrip").nextAll("span:last").text("课程描述字符数应在5～25000之间").show();
		$("body").scrollTop($("#discripContainer").offset().top-100);
		return false;
	} else {
...
//保存课程
function SubmitProductInfo(type, ADMIN_MAIN,SHOP_MAIN) {//alert("1111");
	img_id_arr = "";// 课程主图
		...
		$.ajax({
			...
			success : function(res) {
				...
				if (res != null) {
					...
					showMessage('success', "课程发布成功",__URL(ADMIN_MAIN +'/course/courselist'));
				} else {
					showMessage('error', "课程发布失败",url);
					flag = false;
...
function PackageProductInfo() {
	...
	productViewObj.goodsId = $("#goodsId").val();// 课程id 11号目前为死值 0
	productViewObj.title = $("#txtProductTitle").val().replace(/^\s*/g, "").replace(/\s*$/g, "");// 课程标题
	productViewObj.goods_type = $(".goods_type_select .goods_type_item.selected").attr("data-goods-type");//课程分类
	productViewObj.introduction = $("#txtIntroduction").val().replace(/^\s*/g, "").replace(/\s*$/g, "");// 课程简介，促销语
	
	productViewObj.categoryId = $("#tbcNameCategory").attr("cid");// 课程类目 
	...
	productViewObj.categoryExtendId = category_extend_id;// 课程扩展类目
	// 12号 课程类目；
	...
	productViewObj.imageArray = imageVals;// 课程图片分组
...
function addExtentCategoryBox(){
	...
	html += '<span class="help-inline" style="vertical-align: top;">已添加的课程扩展分类不能为空</span>';
