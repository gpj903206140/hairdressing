...
$(function(){
	...
	$(".js-confirm-refund-ok").click(function(){
		...
		if(validation()){
			...
			$("#countdown_refund_confirm").attr("data-id",id);
	...
	$("#countdown_refund_confirm").click(function(){
		if(!$(this).hasClass("disabled") && $(this).attr("data-id")  && $(this).attr("data-order-id") && $(this).attr("data-order-goods-id")){
			orderGoodsConfirmRefund($(this).attr("data-id"),$(this).attr("data-order-id"),$(this).attr("data-order-goods-id"),$(this).attr("data-refund-money"),$("#balance_refund").attr("data-refund-balance-money"));
		}
	});
})
/**
...
//退款
function orderGoodsConfirmRefund(id, order_id,order_goods_id,refund_money,refund_balance_money){
//	console.log(order_id+","+order_goods_id+","+refund_money+","+refund_balance_money);
	...
	$.ajax({
		type : "post",
		url : __URL(ADMINMAIN + "/order/ordercustomerconfirmrefund"),
		data : {'id': id ,'order_id':order_id,"order_goods_id":order_goods_id, "refund_real_money":refund_money, "refund_balance_money" : refund_balance_money, "refund_way" : $("#refund_way_select").val(), "refund_remark" : $("#refund-remark").val() },
		success : function(data) {
 ...
 */
function confirmRefund(id, order_id,order_goods_id,refund_require_money){
	if(countdown_btn != null){
		clearInterval(countdown_btn);
