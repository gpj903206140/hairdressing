...
class BasesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $voucher2 = Session::get('voucher');
        $controllerName = request()->controller();
        if (empty($voucher)&&!isset($voucher2)&&empty($voucher2)) {
            if($controllerName!='FrontDesk'){
        ...
        //http://localhost/shopping9/index.php?s=/shop/index/index&voucher=21
        // 请求端（PC端、手机端）
        // getShopCache();//开启缓存
        parent::__construct();
        $this->init();
        ...
        $this->assign("call_list", $call_list['data'][0]);
        //底部信息
        $WebSite = new WebSiteModel();
        $website = $WebSite->getInfo([],'title,web_icp,web_address');
        $this->assign("website", $website);
        $shop_id = $this->instance_id;
        $config_service = new WebConfig();
        $config_list = $config_service->getcustomserviceConfig($shop_id);
        if (empty($list)) {
            $list['id'] = '';
    ...
    public function init()
    {
        ...
        if (! request()->isAjax()) {
            ...
            if (empty($web_info['web_popup_title'])) {
                $this->assign("web_popup_title", "商城");
            } else {
