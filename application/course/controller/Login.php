...
class Login extends Controller
{
    ...
    public function register()
    {
        ...
        if (request()->isAjax()) {
           ...
           if($retval_id>0){
                ...
                if(!empty($arr)){
                   if($Relations->saveAll($arr)){
                      $a=$Relations->getInfo(['ChildId'=>$retval_id]);
                      $Relations->commit();
                   }
                }
            }            
    ...
    public function code_register()
    {
        ...
        if (request()->isAjax()) {
            ...
            return view($this->style . 'Login/code_register');
        }
    }
    // 判断手机号存在不
    ...
    public function forgotValidation()
    {
        ...
        if (empty($result)) {
            ...
            return $result = [
                'code' => $result['code'],
                'message' => $result['message']//$result['errorMsg']
            ];
        }
    }
