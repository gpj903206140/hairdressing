...
class Member extends BaseController
{
    ...
    public function memberIndex()
    {
       ...
       /* $is_open_virtual_goods = $this->getIsOpenVirtualGoodsConfig($this->instance_id);
        ...
        //帮助中心
        $member_menu_arr["help"] = array(
                lang('help'),
                'Member/help'
            );
        //关于我们
        $member_menu_arr["about"] = array(
                lang('about'),
                'Member/about'
            );
        ...
        $is_promoters = $user->getInfo(['uid'=>$uid], 'is_promoters,total_commission,stop_commission,can_commission');
        
        $relations = new DistributionRelations();
        //我的团队
        $res['teamNum'] = $relations->getCount(['Pid'=>$uid])+1;
        //我的佣金
        $res['commission'] = $is_promoters['can_commission']; 
        $this->assign('res',$res);
    ...
    public function help(){
        ...
        return view($this->style.'Member/help');
    }
    //关于我们
    ...
    function showShopQecode()
    {
        ...
        $instance_id = $this->instance_id;
        if ($instance_id == 0) {
            $url = __URL(__URL__ . '/course?source_uid=' . $uid);
        } else {
            $url = __URL(__URL__ . '/course/shop/index?shop_id=' . $instance_id . '&source_uid=' . $uid);
        }
        // 查询并生成二维码
        ...
        if (request()->isAjax()) {
            ...
            if ($this->login_verify_code["value"]["pc"] == 1) {
                ...
                if ($type == 'email') {
                    ...
                    //$hook = runhook('Notify', 'bindMobile', $params);
                    $sms = new smsSend();
                    $hook = $sms->getSms($params['mobile']);
                    Session::set('VerificationCode', $hook['param']);
                }
                
                if (! empty($hook['msgId']) && ! empty($hook['param'])) {
