...
use data\model\NsOrderGoodsModel as NsOrderGoods;
...
class Order extends BaseController
{
     ...
     * 收银记录
     */
    public function collectMoneyList()
    {
        if (request()->isAjax()) {
            $page_index = request()->post("page_index", 1);
            $page_size = request()->post("page_size", PAGESIZE);
            $order_no = request()->post("order_no", 1);
            $nick_name = request()->post("nick_name", 1);
            $payment_method = request()->post("payment_method", 1);
            ...
            $condition['order_no'] = array("like","%" . $order_no . "%");
            $condition['nick_name'] = array("like","%" . $nick_name . "%");
            $result = $order_service->collectMoneyList($page_index, $page_size, $condition, "create_time desc");
            foreach($result['data'] as $k=>$v){
                $result['data'][$k]['count'] = $ContinuedMoney->getCount(['collect_id'=>$v['collect_id']]);
            }

            return $result; 
        } else {
            
            return view($this->style . "Order/collectMoneyList");
        }
    }
    /**
     * 收银详情
    ...
    public function collectDetail()
    {
        if (request()->isAjax()) {
            ...
            $result = $CollectMoney->getInfo(['collect_id'=>$collect_id],'*');
            return $result; 
        } else {
     ...
     * 金额添加
     */
    public function addContinuedMoney()
    {
        $order_service = new OrderService();
        $collect_id = request()->post('collect_id',0);
        ...
        $create_time = time();
        $res = $order_service->addContinuedMoney($collect_id, $money, $remark, $create_time);
        return $res;
    }
    /**
     * 金额查询
     */
    public function getWhatMoney()
    {
        $ContinuedMoney = new ContinuedMoney();
        $collect_id = request()->post('collect_id',0);
        $list = $ContinuedMoney->getQuery(['collect_id'=>$collect_id],'*','create_time');
        if(empty($list)){
            return 0;
        }
        return $list;
     ...
     * 收银添加
     */
    public function addCollectMoney()
    {
        if (request()->isAjax()) {
            $order_service = new OrderService();
            $collect_id = request()->post('collect_id',0);
            ...
            $reserve_phone = request()->post('reserve_phone', '');
            $uid = request()->post('uid', '');
            $nick_name = request()->post('nick_name', '');
            $money = request()->post('money', '');
            $meet_money = request()->post('meet_money', '');
            $coupon_name = request()->post('coupon_name', '');
            $coupon_id = request()->post('coupon_id', '');
            $coupon_money = request()->post('coupon_money', '');
            $at_leasts = request()->post("at_leasts", "");
            $realPayment_money = request()->post('realPayment_money', '');
            $payment_method = request()->post('payment_method', '');
            $accountsWx_money = request()->post('accountsWx_money', '');
            $remark = request()->post('remark', '');
            $paymentDate = request()->post("paymentDate", "");
            $create_time = time();
            $res = $order_service->updateCollectMoney($collect_id, $order_id, $goods_id, $goods_name, $reserve_phone, $order_no, $uid, $nick_name, $money, $meet_money, $coupon_name, $coupon_id, $coupon_money, $at_leasts, $realPayment_money, $payment_method, $accountsWx_money, $paymentDate, $remark, $create_time);
            return $res;
        } else {
            $order_no = request()->get('order_no','');
            $data = $this->getOrderNo_user($order_no);
            $this->assign('data',$data);
            return view($this->style . "Order/addCollectMoney");
        }
    }
    /**
     * 收银修改
     */
    public function updateCollectMoney()
    {
        if (request()->isAjax()) {
            ...
            $CollectMoney = new CollectMoney();
            $data = $CollectMoney->getInfo(['collect_id'=>$collect_id],'*');
            $this->assign('data',$data);
            return view($this->style . "Order/updateCollectMoney");
        }
    }
    /**
     * 收银编辑-通过预约订单号查到下单人
     */
    public function getOrderNo_user($order_no='')
    {
        $order = new NsOrderModel();
        $OrderGoods = new NsOrderGoods();
        if(empty($order_no)){
            $order_no = request()->post('order_no', 1);
        }
        $row = $order->getInfo(['order_no'=>$order_no,'order_status'=>10,'pay_status'=>['not in','-10,-11']],'order_id,user_name,receiver_mobile,order_no');
        if(!empty($row)){
            $user = new user();
            $result = $OrderGoods->getInfo(['order_id'=>$row['order_id']],'goods_id,buyer_id,goods_name');
            $result['order_id'] = $row['order_id'];
            $result['nick_name'] = $row['user_name'];
            $result['receiver_mobile'] = $row['receiver_mobile'];
            $result['order_no'] = $row['order_no'];
            //查找课程购买者的会员等级和可优惠折扣
            $member = new NsMemberModel();
            $member_level = new NsMemberLevelModel();
            $Level = $member->getInfo(['uid'=>$result['buyer_id']],'member_level');
            $goods_discount = $member_level->getInfo(['level_id'=>$Level['member_level']],'goods_discount,level_name');
            if(empty($goods_discount)){
                $result['goods_discount'] = 1;
                $result['level_name'] = '';
            }else{
                $result['goods_discount'] = $goods_discount['goods_discount'];
                $result['level_name'] = $goods_discount['level_name'];
            }
            return $result;
        }else{
            echo 0;
    ...
    /**
     * 收银编辑-通过下单人Id找到可使用的优惠券
     */
    public function getCoupon_user()
    {
        $coupon = new NsCouponModel();
        $couponType = new NsCouponTypeModel();
        $uid = request()->post('uid', 0);
        $data = $coupon->getQuery(['uid'=>$uid,'state'=>1],'coupon_id,coupon_type_id,money,start_time,end_time,fetch_time','fetch_time desc');
        if(!empty($data)){
            foreach($data as $k=>$v){
                $con = $couponType->getInfo(['coupon_type_id'=>$v['coupon_type_id']],'at_least,coupon_name');
                $data[$k]['coupon_name'] = $con['coupon_name'];
                $data[$k]['at_least'] = $con['at_least'];
            }
            return $data;
        }else{
            echo 0;
        ...
        if (request()->isAjax()) {
            ...
            foreach($list['data'] as $k=>$v){
                if($v['order_status']==10){
                    $list['data'][$k]['collect'] = $CollectMoney->getInfo(['order_no'=>$v['order_no']],'meet_money,coupon_money,realPayment_money,payment_method');
                }
            }
            return $list;
        } else {
    ...
    public function orderRefundDetail()
    {
        ...
        if($info['order_status']==10){
            return view($this->style . "Order/orderReservationDetail");
        }else{
    ...
    public function promoterExcel()
    {
        ...
        $collect_ids = request()->get("collect_ids", "");
        if ($collect_ids != "") {
            $condition["collect_id"] = [
                "in",
                $collect_ids
            ];
        }
        ...
        $CollectMoney = new CollectMoney();
        $ContinuedMoney = new ContinuedMoney();
        $list = $CollectMoney->getQuery($condition, '*','create_time desc');
        foreach ($list as $k => $v) {
            ...
            $list[$k]["payment_method"] = $payment_method;
            $ContinuedList = $ContinuedMoney->getQuery(['collect_id'=>$v['collect_id']],'money,remark,create_time','create_time desc');
            $info = "";
            if(!empty($ContinuedList)){
                foreach ($ContinuedList as $t => $m) {
                    $info .= "金额:" . $m["money"] . "    备注:" . $m["remark"] . "   添加时间: ".getTimeStampTurnTime($m["create_time"])."\r\n";
                }
            }
            $list[$k]["coupon_name"] = $info;
        }
        dataExcel($xlsName, $xlsCell, $list);
    }
    public function orderDataExcel()
    {
        ...
        foreach ($list as $k => $v) {
            ...
            $v["receiver_address"] = str_replace('&nbsp;',' ',$v["receiver_address"]);
            ...
            if ($v['pay_status'] == 0 && $v['order_status'] != 10) {
                $list[$k]["pay_status_name"] = '待付款';
            } elseif ($v['pay_status'] == 2 && $v['order_status'] != 10) {
                $list[$k]["pay_status_name"] = '已付款';
            } elseif ($v['pay_status'] == 1 && $v['order_status'] != 10) {
                $list[$k]["pay_status_name"] = '支付中';
            }else if ($v['order_status'] == 10){
                if ($v['pay_status'] == 10) {
                    $list[$k]["pay_status_name"] = '已完成';
                }elseif ($v['pay_status'] == -11) {
                    $list[$k]["pay_status_name"] = '取消订单申请中';
                }elseif ($v['pay_status'] == -10) {
                    $list[$k]["pay_status_name"] = '已关闭';
                }else{
                    $list[$k]["pay_status_name"] = '未完成';
                }
                $collect = $CollectMoney->getInfo(['order_no'=>$v['order_no']],'meet_money,coupon_money,realPayment_money,payment_method');
                switch ($collect['payment_method']) {
                     case 0:
                         $payment_method = '支付宝';
                         break;
                     case 1:
                ...
                $list[$k]["pay_type_name"] = $payment_method;
                $list[$k]["pay_money"] = $collect['realPayment_money'];
                $list[$k]["receiver_info"] = '预约时间:'.$v["shipping_time"].'  预约手机号: '.$v["receiver_mobile"];
                $list[$k]["status_name"] = '';
            ...
            if ($v['order_status'] == 10){
                foreach ($v["order_item_list"] as $t => $m) {
                    $goods_info .= "商品名称:" . $m["goods_name"] . "   商品价格:" . $m["price"] . "  人数:" . $m["num"] . "  ";
                }
            }else{
                foreach ($v["order_item_list"] as $t => $m) {
    ...
    {
        $ali_pay = new AliPay();
        $retval = $ali_pay->aliPayTransfer(date("YmdHis", time()) . rand(100000, 999999), '123456@qq.com', 1);
        $this->redirect($retval);
    }
