...
namespace app\adminsite\controller;
 {if condition='$k neq 0 && ($k+1)%5==0'}last{/if}
...
class Order extends BaseController
{
    ...
    public function addCollectMoney()
    {
        if (request()->isAjax()) {
            ...
            $create_time = time();
            $res = $order_service->updateCollectMoney($collect_id, $order_id, $goods_id, $goods_name, $reserve_phone, $order_no, $uid, $nick_name, $meet_money, $coupon_name, $coupon_id, $coupon_money, $at_leasts, $realPayment_money, $payment_method, $accountsWx_money, $paymentDate, $remark, $create_time);
            return $res;
        } else {
     ...
     * 收银编辑-通过预约订单号查到下单人
     */
    public function getOrderNo_user()
    {
        $order = new NsOrderModel();
        $OrderGoods = new NsOrderGoods();
        $order_no = request()->post('order_no', 1);
        $row = $order->getInfo(['order_no'=>$order_no,'order_status'=>10],'order_id,user_name');
        if(!empty($row)){
            $user = new user();
