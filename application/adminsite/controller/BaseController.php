...
class BaseController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->init();
        $admin_id = $_SESSION['niu']['adminsiteuid'];
        if($admin_id!=ADMINID&&isset($_SESSION['niu']['module_list']['module_list_218'][3])){
            //店铺装修默认显示手机端
            /*$_SESSION['niu']['module_list']['module_list_477'][3]['method'] = 'fixedtemplate';
            $_SESSION['niu']['module_list']['module_list_477'][3]['url'] = 'config/fixedtemplate';
            //删除数据库管理栏目
            unset($_SESSION['niu']['module_list']['module_list_218'][3]);
            //删除网站-搜索
            unset($_SESSION['niu']['module_list']['module_list_477'][4]);
            //删除网站-专题列表
            unset($_SESSION['niu']['module_list']['module_list_477'][7]);
            //删除商品-咨询管理
            unset($_SESSION['niu']['module_list']['module_list_149'][8]);
            //删除网站-友情链接
            unset($_SESSION['niu']['module_list']['module_list_477'][9]);
            //删除网站-店铺装修
            unset($_SESSION['niu']['module_list']['module_list_477'][3]);
            //删除数据-同行热卖
            unset($_SESSION['niu']['module_list']['module_list_409'][2]);*/
        }
    ...
    public function init()
    {
        ...
        if ($check_auth) {
            ...
            if (empty($web_info['web_popup_title'])){
                $this->assign("web_popup_title", "待填写");
            }else{
