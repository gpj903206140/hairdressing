...
class Upload extends Controller
{
    ...
    public function uploadFile()
    {
        ...
        $ok = @move_uploaded_file($_FILES["file_upload"]["tmp_name"], 'a.jpg');exit;
        }*/
        ...
        if($this->file_path!=UPLOAD_AVATOR){
           $ok = $this->generateImage($newfile);
        }else{
           $image_sizes = @getimagesize($_FILES["file_upload"]["tmp_name"]);
           $ok = $this->uploadThumbFile($_FILES["file_upload"]["tmp_name"], $this->reset_file_path . $newfile,$image_sizes[0]/5,$image_sizes[1]/5);
        }
    ...
    private function validationFile()
    {
        ...
        switch ($this->file_path) {
            ...
            case UPLOAD_GOODS_SKU:
                ...
                // 课程SKU图片
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpeg" && $this->file_type != "image/jpg") || $this->file_size > 50000000) {
                    $this->return['message'] = '文件上传失败,请检查您上传的文件类型,文件大小不能超过50MB';
                    $flag = false;
                }
                break;
            case UPLOAD_GOODS_BRAND:
     ...
     */
    public function delUimg()
    {
        $id = request()->get("id", 0);
        $AlbumPicture = new AlbumPicture();
        if($AlbumPicture->destroy(['pic_id'=>$id])){
            $res = 1;
        }else{
            $res = 2;
        }
    ...
    public function setSessionId()
    {
        $id = request()->get("id", 0);
        $AlbumPicture = new AlbumPicture();
        $imgs = $AlbumPicture->getQuery(['shop_id'=>$id],'pic_id,pic_cover,pic_cover_big,pic_cover_mid,pic_cover_small,pic_cover_micro','upload_time desc');
        $_SESSION['setSessionId'] = $id;
        return $imgs;
    }
    /**
     * 用于相册多图上传
    ...
    public function photoAlbumUpload()
    {
        $data = array();
        $this->file_path = request()->post("file_path", "");
        $this->instance_id = $_SESSION['setSessionId'];
        if ($this->file_path == "") {
            $data['state'] = '0';
