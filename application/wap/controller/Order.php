...
class Order extends BaseController
{
    ...
    public function orderCreateReservation()
    {
        ...
        $shipping_time = request()->post('shipping_time', 0); // 预约时间
        $num = request()->post('num', ''); // 预约人数
        $receiver_mobile = request()->post('receiver_mobile', ''); // 预留手机号
        $buyer_message = request()->post("buyer_message", 0); // 备注
        $goods_sku_list = $goods_id.':1';  
        $time = time();
        $stime = strtotime($shipping_time);
        if($time>$stime){
            $res = array(
                "code" => 0,
        ...
        // 查询商品限购
        $purchase_restriction = $order->getGoodsStock($goods_id);
        if (! empty($purchase_restriction)) {
            $res = array(
                "code" => 0,
                "message" => $purchase_restriction
            ...
            $order_id = $order->orderCreateReservation($out_trade_no, $goods_id, $use_coupon, $shipping_time, $num, $receiver_mobile, $buyer_message);
            /*//通过销售总额判断是否升级推广员等级
            $order->updateCommissionDistributionGrade();
            //记录订单的分销佣金
            $order->updateCommissionDistributionIssue($order_id);
