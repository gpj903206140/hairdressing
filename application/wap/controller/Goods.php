...
use data\model\NcCmsArticleClassModel as cmsArticleClass;
use data\model\NcCmsArticleCommentModel as cmsArticleComment;
use data\model\NcCmsCommentZanModel as cmsCommentZan;
...
class Goods extends BaseController
{
    ...
    public function articleList(){
        if(request()->isAjax()){
            ...
            $data = $cmsArticle->pageQuery($page,$offset,$where,'commend_flag desc,sort,create_time desc','article_id,title,summary,image,click,create_time,commend_flag');
            return $data;
        }else{
            //查询资讯分类
            $cmsArticleClass = new cmsArticleClass();
            $class = $cmsArticleClass->getQuery([],'*','sort desc');
            $this->assign('class',$class);
            return view($this->style . 'Goods/articleList');
        }
    }
    //资讯收藏列表
    public function articleCollectionList(){
        $uid = $this->uid;
        if(request()->isAjax()){
            // 资讯中心
            $comment = new cmsArticleComment();
            $cmsArticle = new cmsArticle();
            $commentQuery = $comment->getQuery(['uid'=>$uid,'type'=>1],'article_id','create_time desc');
            ...
            $where = "status=2 and article_id in({$article_id})";
            if($class_id>0){
                $where .= " and class_id={$class_id}";
            }
            $data = $cmsArticle->pageQuery($page,$offset,$where,'create_time desc','article_id,title,summary,image,click,create_time');
            return $data;
        }else{
            //查询资讯分类
            $cmsArticleClass = new cmsArticleClass();
            $class = $cmsArticleClass->getQuery([],'*','sort desc');
            $this->assign('class',$class);
            return view($this->style . 'Goods/articleCollectionList');
        }
    }
    //评论列表
    public function commentList(){
        $cmsArticle = new cmsArticle();
        $comment = new cmsArticleComment();
        $cmsCommentZan = new cmsCommentZan();
        $uid = $this->uid;
        if(request()->isAjax()){
            $page_num = 5;
            $page = request()->post('page', 1);
            $article_id = request()->post('article_id', 1);
            $offset = $page<=1?$page_num*($page-1):$page_num*($page-1);
            $where = " a.article_id={$article_id} and a.is_show=1 and type=0 and a.uid=b.uid";
            $field = 'a.*,b.user_headimg';
            $data['data'] = Dbs::table('nc_cms_article_comment a,sys_user b')->field($field)->where($where)->limit($offset.','.$page_num)->order('a.create_time desc')->select();
            foreach($data['data'] as $k=>$v){
                $data['data'][$k]['count'] = $cmsCommentZan->getCount(['uid'=>$uid,'comment_id'=>$v['comment_id']]);
            }
            $count = Dbs::table('nc_cms_article_comment')->field('comment_id')->where(['article_id'=>$article_id,'type'=>0])->count();
            ...
            return view($this->style . 'Goods/commentList');
        }
    }
    //资讯详情
    public function articleDetail(){
        ...
        if(request()->isAjax()){
            ...
            $datatype = request()->post('datatype', 0);
            $is_show = request()->post('is_show', 0);
            if (! empty($this->uid)) {
                $goods = new GoodsService();
                $user = new User();
                $nick_name = $user->getInfo(['uid'=>$uid],'nick_name');
                $retval = $goods->addComment($uid, $nick_name['nick_name'], $article_id, $content,$type,$datatype,$is_show);
                $retval['create_time'] = time();
            } else {
                $retval = array(
                    "code" => - 1,
                    "message" => ""
            ...
            $user = new User();
            $article_id = request()->get('article_id',0);
            $condition = ['article_id'=>$article_id,'type'=>0,'is_show'=>1];
            $cmsDetail = $cmsArticle->getInfo(['article_id'=>$article_id],'*');
            $datatype = $comment->getInfo(['article_id'=>$article_id,'uid'=>$uid,'type'=>1],'comment_id');
            if(!$uid){
               $cmsDetail['comment_flag'] = 0; 
            }
            //评价内容取最新10条
            $list = $comment->pageQuery(1, 10, $condition, 'create_time desc', '*');
            foreach($list['data'] as $k=>$v){
                $list['data'][$k]['count'] = $cmsCommentZan->getCount(['uid'=>$uid,'comment_id'=>$v['comment_id']]);
                $headimg = $user->getInfo(['uid'=>$v['uid']],'user_headimg');
                $list['data'][$k]['headimg'] = '';
                if(!empty($headimg)){
                    $list['data'][$k]['headimg'] = $headimg['user_headimg'];
                }
            }
            //取用户信息
            $userCon = $user->getInfo(['uid'=>$uid],'uid,nick_name,user_headimg');
            $this->assign('userCon', $userCon);
            $this->assign('list', $list);
            $this->assign('datatype', $datatype);
            $this->assign('cmsDetail', $cmsDetail);
            return view($this->style . 'Goods/articleDetail');
        }
    }
    //查看文章 点击数加+
    public function addClick(){
        $cmsArticle = new cmsArticle();
        $article_id = request()->get('article_id', 0);
        $retval = $cmsArticle->where('article_id',$article_id)->setInc('click');
        return $retval;
    }
    //文章评论点赞&取消点赞
    public function updatezan(){
        $uid = $this->uid;
        ...
        if (! empty($this->uid)) {
            ...
            $nick_name = $user->getInfo(['uid'=>$uid],'nick_name');
            $data['uid'] = $uid;
            $data['nick_name'] = $nick_name['nick_name'];
            $data['comment_id'] = $comment_id;
            $data['create_time'] = time();
            $where = ['uid'=>$uid,'comment_id'=>$comment_id];
            $count = $cmsCommentZan->getCount($where);
            $cmsCommentZan->startTrans();
            if($count>0){
                $result1 = $cmsCommentZan->destroy($where);
                $result2 = $comment->where('comment_id',$comment_id)->setDec('zan_num',1);
                $message = '取消点赞';
            }else{
                $result1 = $cmsCommentZan->save($data);
                $result2 = $comment->where('comment_id',$comment_id)->setInc('zan_num',1);
                $message = '点赞';
            }
            if($result1&&$result2){
                $retval['code'] = 1;
                $retval['message'] = $message.'成功';
                $cmsCommentZan->commit();
            }else{
                $retval['code'] = -1;
                $retval['message'] = $message.'失败';
                $cmsCommentZan->rollback(); 
            }
        } else {
            $retval = array(
                "code" => - 1,
    ...
    public function goodsDetail()
    {
        ...
        // 获取商品的优惠劵
        $goods_coupon_list = $goods->getGoodsCoupon($goods_id, $this->uid, $goods_detail['goods_kind']);
        ...
        if(empty($goods_detail["wap_custom_template"])){
            ...
            if ($is_support_pintuan == 1) {
                ...
                if ($goods_pintuan['is_open']) {
                    ...
                    $this->assign("tuangou_group_count", $tuangou_group_count);echo 'aa';
                ...
                if($goods_detail["goods_kind"]==1){
                     return view($this->style . 'Goods/goodsDetailBespoke');
                }else{
                    if($goods_detail["point_exchange_type"] == 0 || $goods_detail["point_exchange_type"] == 2|| $goods_detail["is_open_presell"] == 1){
                        return view($this->style . 'Goods/goodsDetail');
                    }else{
    ...
    /*public function goods_reservation()
    {
        ...
        $this->assign("goods_coupon_list", $goods_coupon_list);
        return view($this->style . 'Goods/goods_reservation');
    }*/
    /**
     * 根据定位查询当前商品的运费
     */
